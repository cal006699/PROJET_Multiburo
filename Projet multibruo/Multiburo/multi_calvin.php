...
<head>
    ...
    ?>
        ...
        <div class="container">
            <h1>Multiburo</h1>
            <?php //permet d'initialisé la date d'aujourd'hui si aucune date n'a étais selectionné
            if (isset($_POST['date'])) {
                $dateAffichee = $_POST['date'];
            } else {
                $dateAffichee = date('Y-m-d');
            }?>

            
            <form action="multi_calvin.php" method="post">
                <input type="date" 
                    value="<?php echo $dateAffichee; ?>" 
                    name="date" 
